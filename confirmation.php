...
<html>
	...
	<body>
		...
		<?php
			...
			if (isset($_GET['token'])) {
				$token = $_GET['token'];
				$user = mysqli_query($bdd, "SELECT * FROM users WHERE USR_Token = '" . $token. "'");
				if (mysqli_num_rows($user) > 0) {
					$fetch = mysqli_fetch_assoc($user);
					if ($fetch['USR_Confirmed'] == 0) {
						$ret = mysqli_query($bdd, "UPDATE users SET USR_Confirmed = '1' WHERE USR_Token = '$token'");
						echo 'Compte confirme.';
					} else {
						echo "Compte deja confirme.";
					}
				} else {
					echo "Token invalide.";
				}
				...
				echo "Aucun Token de confirmation.";
			}
		?>
		</div>
	</body>
